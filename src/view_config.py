...
async def history():
    ...
    return render_template(
        'index_all.html', group='group', data=data, regex='regex', channels='channels', regex_download='_regex_download', folder_download='_folder_download', regex_rename='_regex_rename', rename='rename', downloaded=downloaded
    )
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            for d in data:
                ...
                newdata.id             = d.id
                newdata.date           = d.date
                newdata.file_name      = d.video.file_name
                newdata.caption        = d.caption 
                newdata.width          = f"{d.video.width}x{d.video.height}"
